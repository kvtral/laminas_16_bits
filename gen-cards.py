#!/usr/bin/env python3
# Genera cards.json escaneando images/*.jpeg. Re-correr al agregar láminas.
# Uso: python3 gen-cards.py [BASE_URL]   (ej: https://kvtral.github.io/laminas_16_bits/)
...
laminas, huerfanas = [], []
for path in sorted(glob.glob("images/*.jpeg")):
    file = os.path.basename(path)
    key = file[:2]
    if key not in KEYS:
        huerfanas.append(file); continue
    laminas.append({
        "categoria": key,
        "nombre": pretty(file[:-5]),
        "file": file,
        "url": BASE + "images/" + file,   # link absoluto para consumo externo
    })
...
if huerfanas:
    print("OJO, prefijo desconocido (ignoradas):", huerfanas)
